...
const StackingCardItem = ({
  ...
  const top = topPosition ?? `${index * 20}px`

  return (
    <div className={cn("sticky top-0 h-screen w-full flex items-center justify-center", className)} {...props}>
      <motion.div
        className={"origin-top relative w-full"}
        style={{ top, scale }}
      >
        {children}
      </motion.div>

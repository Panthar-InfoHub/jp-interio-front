...
const redesignSchema = z.object({
  description: z.string().min(10, "Please describe your desired design style (at least 10 characters)"),
  imageFile: z
    .instanceof(File)
    .refine(
    ...
    .refine((file) => file.size <= 10 * 1024 * 1024, "Image must be smaller than 10MB"),
})
    ...
    onSubmit: async ({ value }) => {
      startTransition(async () => {
        try {
          ...
          const response = await redesignImage({ image_uri: gs_uri, stylePrompt: value.description })
      ...
      <div className="bg-card border border-border rounded-lg p-8 md:p-10 shadow-sm">
        ...
        >
          ...
          <form.Field
            ...
            children={(field) => (
              <div className="space-y-3">
                <FieldLabel htmlFor="description">Design Style Description</FieldLabel>
                <Textarea
                  id="description"
                ...
                <p className="text-xs text-muted-foreground">
                  Be specific about colors, materials, mood, and architectural elements
                </p>
                {field.state.meta.errors.length > 0 && <FieldError errors={field.state.meta.errors} />}
              </div>
            )}
          ...
          {/* Submit Button */}
          <Button type="submit" disabled={isPending} className="w-full h-12 text-base font-medium">
            {isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            {isPending ? "Generating Design..." : "Redesign It"}
          </Button>
        </form>
      </div>
